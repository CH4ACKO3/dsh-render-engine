...
The `Publish packages` GitHub Actions workflow publishes the three packages manually, in dependency order, using npm Trusted Publishing (OIDC). Trusted Publishing is configured for all three packages, and no long-lived npm token is stored in GitHub.
